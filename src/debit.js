// Elements & State Variables
    ...
    toggleCardNumberBtn.addEventListener('click', () => {
      ...
      cardNumberEl.textContent = showCardNumber ? '4242 •••• •••• 8812' : '•••• •••• •••• 8812';
    ...
    freezeCardBtn.addEventListener('click', () => {
      ...
      if (isFrozen) {
        ...
        freezeCardBtn.className = 'px-4 py-2 rounded-xl text-xs font-bold transition-all border bg-sky-500 text-white border-sky-400 cursor-pointer';
        freezeCardBtn.textContent = '❄️ Card Frozen (Click to Unfreeze)';
      } else {
        // Active State Styling
        cardGraphicEl.className = 'w-full max-w-sm h-56 rounded-3xl p-6 flex flex-col justify-between shadow-2xl relative overflow-hidden transition-all duration-300 bg-gradient-to-br from-slate-950 via-slate-900 to-[#D32F2F] border border-slate-700/80 shadow-red-500/20';
        freezeCardBtn.className = 'px-4 py-2 rounded-xl text-xs font-bold transition-all border bg-slate-800 text-slate-300 border-slate-700 hover:text-white cursor-pointer';
    ...
    orderCardBtn.addEventListener('click', () => {
      alert('Your virtual Hidmona Card request has been initiated!');
    });